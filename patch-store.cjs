...
// We need to add a function to sync settings to the server
const updateSettingsMatch = `  const updateSettings = (newSettings: Partial<ApplicationSettings>) => {
    setState(prev => ({
      ...prev,
...
const updateSettingsReplace = `  const updateSettings = (newSettings: Partial<ApplicationSettings>) => {
    setState(prev => ({
      ...prev,
      settings: { ...prev.settings, ...newSettings }
    }));
  ...
  const syncSettingsToServer = () => {
    ...
    Object.keys(currentSettings).forEach(key => {
      sheetData.append(key, String(currentSettings[key as keyof ApplicationSettings]));
    });
...
code = code.replace(updateSettingsMatch, updateSettingsReplace);

// add to context
const contextTypeMatch = `  updateSettings: (settings: Partial<ApplicationSettings>) => void;`;
const contextTypeReplace = `  updateSettings: (settings: Partial<ApplicationSettings>) => void;
  syncSettingsToServer: () => void;`;
code = code.replace(contextTypeMatch, contextTypeReplace);

// add to return
const returnMatch = `    <AppContext.Provider value={{
      ...state,
      updateSettings,`;
const returnReplace = `    <AppContext.Provider value={{
      ...state,
      updateSettings,
